...
// Obtener actividades de este curso
...
// Función para verificar el estado de la tarea (CORREGIDA)
function verificarEstadoTarea($conn, $actividad_id, $alumno_id) {
    // Verificar si ya se envió la tarea y obtener calificación de la tabla evaluaciones
    ...
    if ($res_envio && mysqli_num_rows($res_envio) > 0) {
        ...
        // Contar cuántas entregas ha hecho el alumno para esta actividad
        ...
        if ($intentos_usados >= $intentos_permitidos) {
            // Ya se gastaron todos los intentos
            return [
                'estado' => 'bloqueada',
        ...
        // No se ha enviado la tarea todavía
        return [
            'estado' => 'disponible',
...
<head>
    ...
    <title><?php echo htmlspecialchars($curso['nombre']); ?> - D&F Mindspace</title>
                ...
                <div class="modal-content" style="border-radius: 25px; overflow: hidden; border: none;">
                    ...
                    <div class="modal-body p-4">
                        ...
                        <div class="row mb-4">
                            ...
                            <div class="col-6">
                                <div class="d-flex align-items-center gap-2 mb-2">
                                    ...
                                    <div>
                                        ...
                                        <strong id="previewDifficulty"></strong>
                                    </div>
                                </div>
                            </div>
                            <div class="col-6">
                            ...
                            <div class="col-6">
                                <div class="d-flex align-items-center gap-2 mb-2">
                                    ...
                                    <div>
                                        ...
                                        <strong id="previewPoints"></strong>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="col-12 mt-2" id="previewGradeContainer" style="display: none;">
                                <div class="d-flex align-items-center gap-2">
                        ...
                        <div class="text-center">
                            <a href="#" id="previewStartBtn" class="btn-start" style="padding: 12px 30px; border-radius: 15px; text-decoration: none; display: inline-flex; align-items: center; gap: 8px;">
    ...
    <script>
        ...
        previewButtons.forEach(btn => {
            btn.addEventListener('click', function() {
                ...
                const missionTitle = this.dataset.titulo;
                const missionDescription = this.dataset.descripcion;
                const missionType = this.dataset.tipo;
                const missionDifficulty = this.dataset.dificultad;
                const missionTime = this.dataset.tiempo;
                const missionPoints = this.dataset.puntos;
                const missionStatus = this.dataset.estado;
                const missionAttempts = this.dataset.intentos;
                const missionGrade = this.dataset.calificacion;
                
                document.getElementById('previewTitle').textContent = missionTitle;
                document.getElementById('previewDescription').textContent = missionDescription;
                document.getElementById('previewType').textContent = missionType;
                document.getElementById('previewDifficulty').textContent = missionDifficulty;
                document.getElementById('previewTime').textContent = missionTime + ' minutos';
                document.getElementById('previewPoints').textContent = missionPoints + ' puntos';
                ...
                switch(missionStatus) {
                    case 'bloqueada':
                        statusText = 'Completada (Bloqueada)';
                        statusClass = 'text-danger';
                        break;
                    case 'intentos_disponibles':
                        statusText = 'En progreso (' + missionAttempts + ' intentos restantes)';
                        statusClass = 'text-warning';
                        break;
                    default:
                        statusText = 'Disponible (' + missionAttempts + ' intentos disponibles)';
                        statusClass = 'text-success';
                }
                
                document.getElementById('previewStatus').textContent = statusText;
                document.getElementById('previewStatus').className = statusClass;
                
                // Mostrar calificación si existe
                const gradeContainer = document.getElementById('previewGradeContainer');
                if(missionGrade && missionGrade > 0) {
                    document.getElementById('previewGrade').textContent = missionGrade + '/10';
                    gradeContainer.style.display = 'flex';
                } else {
                    gradeContainer.style.display = 'none';
                }
                
                // Configurar enlace del botón según estado
                const startBtn = document.getElementById('previewStartBtn');
                if(missionStatus === 'bloqueada') {
                    startBtn.href = `entrega.php?id_actividad=${missionId}&id_alumno=<?php echo $alumno_id; ?>`;
                    startBtn.innerHTML = '<i class="fas fa-eye me-2"></i>Ver Entrega';
                    startBtn.className = startBtn.className.replace('btn-start', 'btn-view');
                } else {
                    startBtn.href = `realizar_actividad.php?id=${missionId}`;
                    startBtn.innerHTML = '<i class="fas fa-play me-2"></i>¡Comenzar Misión Ahora!';
                    startBtn.className = startBtn.className.replace('btn-view', 'btn-start');
                    
                    // Si ya hay calificación, cambiar texto del botón
                    if(missionGrade && missionGrade > 0) {
                        startBtn.innerHTML = '<i class="fas fa-redo me-2"></i>Volver a Intentar';
                        startBtn.className = startBtn.className.replace('btn-start', 'btn-retry');
                    }
                }
